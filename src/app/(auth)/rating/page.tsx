...
import {StudentCompareModal} from "@/page/rating/components/StudentCompareModal";
import {Pagination} from "@/shared/ui/Pagination/Pagination";
import {PAGE_BACK_DIFFERENT} from "@/shared/constants"; // Импортируем модальное окно
...
export default function StudentsRatingTable() {
		...
		const [studentsParams, setStudentsParams] = useState<Omit<UserSearchResponse, 'content'>>();
		...
		const [size] = useState(15); // _setSize используется для подавления ошибки, если size не меняется
		...
		const handleSearch = (currentPage = page, currentSize = size) => {
				...
				const requestBody = {
						...
						courseNumber: selectedCourse ?? null,
						mainSkillCategoryId: selectedRole ?? null,
						minAverageRating: selectedRating ?? null,
				};
		...
		return (
				<div className="bg-bg-default min-h-screen">
						<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
								...
								<div className="mt-8">
										<StudentsTable
												studentsCount={studentsParams?.totalElements}
								...
								<div className="mt-8 flex w-full items-center justify-center">
										<Pagination
												page={page + PAGE_BACK_DIFFERENT}
												totalPages={studentsParams?.totalPages || page}
						            onChangePage={(page) => handleSearch(page - PAGE_BACK_DIFFERENT)}
										/>
								</div>
